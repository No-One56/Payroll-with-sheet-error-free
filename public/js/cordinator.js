...
function fetchCoordinatorProjects(userName) {
    fetch('cordinator/projects') // API endpoint
        .then(response => {
            if (!response.ok) {
                throw new Error(`HTTP error! Status: ${response.status}`);
            }
        ...
        .then(data => {
            console.log(`Projects for coordinator ${userName}:`, data.projects);
            populateSupervisionTable(data.projects);
        })
        .catch(error => {
            console.error('Error fetching coordinator projects:', error);
            alert('Error fetching your assigned projects.');
        });
}

function populateSupervisionTable(projects) {
    const tableBody = document.querySelector('#supervision-table tbody');
    tableBody.innerHTML = ''; // Clear the table body before populating new rows
